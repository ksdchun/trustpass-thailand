...
export function languageInstruction(language: Language): string {
  switch (language) {
    ...
    case "Chinese":
      return `# Output language — CRITICAL\nThe tourist chose Simplified Chinese (简体中文). You MUST write the following fields entirely in Simplified Chinese, even if the local_rule_baseline provided is in English — do NOT copy or translate the baseline, write fresh Chinese content:\n- \`why_it_matters\` — explain the risk in plain Chinese a stressed tourist can read\n- \`safe_next_steps[].step\` — every action step in Chinese\n- \`evidence_to_save\` — each item in Chinese\n- \`contact_recommendation\` — in Chinese\n- \`category\` — short Chinese label (e.g. 出租车计价器诈骗, 无强烈诈骗模式)\nDo NOT write English in any of those fields. Keep \`thai_phrase\` in Thai script (shown to Thai speakers). \`incident_report_summary.english\` stays in English. \`incident_report_summary.thai\` stays in Thai.`;
    case "English":
    default:
